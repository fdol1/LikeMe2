...
public class SwipeByCoordinates implements Interaction {
	...
	public <T extends Actor> void performAs(T actor) {
		(new TouchAction(GetDriver.as(actor).appiumDriver())).longPress((new PointOption()).withCoordinates(this.inicialX, this.inicialY)).moveTo((new PointOption()).withCoordinates(this.endX, this.endY)).release().perform();
	}
}
